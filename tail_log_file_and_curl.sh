...
tail -n0 -F "$1" | while read LINE; do
  ...
  #echo $JSON
  curl -X PUT "http://192.168.1.6:9200/max-medical/rad8/$ID" -d "$JSON" &

  SPO2_BPM=`echo $JSON | awk '{print $8" "$10}' | sed 's/,//g'`
  SPO2=`echo $SPO2_BPM | awk '{print $1}'`
  #echo "SPO2=_${SPO2}_"
  if [ "x$SPO2" != "x" ] && [ "x$SPO2" != "x$CURR_SPO2" ]; then
    curl -X GET "https://graph.api.smartthings.com/api/token/$ACCESS_TOKEN/smartapps/installations/$APP_ID/spo2/$SPO2" &
  ...
  if [ "x$BPM" != "x" ] && [ "x$BPM" != "x$CURR_BPM" ]; then                                                                                                                    
    curl -X GET "https://graph.api.smartthings.com/api/token/$ACCESS_TOKEN/smartapps/installations/$APP_ID/bpm/$BPM" &
